...
def test_root_returns_index_html():
    response = client.get("/")
    # In TestClient, static files are not served, but the redirect is followed automatically, so we get 200
    assert response.status_code == 200 or response.status_code in (307, 302)
...
def test_signup_and_unregister():
    ...
    # Ensure not already signed up (ignore result)
    ...
    # Accept 200 (success) or 404 (already removed by previous test run)
    assert response.status_code in (200, 404)
    if response.status_code == 200:
        assert f"Unregistered {test_email}" in response.json()["message"]

    # Unregister again should fail (allow 400 or 404 for idempotency)
    response = client.post(f"/activities/{activity}/unregister?email={test_email}")
